...
class DatePresetPicker extends PureComponent {
	static propTypes = {
		...
		customRangeLabel: PropTypes.oneOfType([PropTypes.object, PropTypes.string]),
	};
	...
	onCalendarFocusChange = (focusedCalendar) => {
		if (this.state.pickerVisible) {
			const newFocus = {
				focusedCalendar,
				presetPickerActive: !focusedCalendar,
				pickerVisible: !!focusedCalendar,
			};
		...
		return (
			<div className={styles.wrapper}>
				<div
					...
					className={classnames(styles.pickerWrapper, className)}
				>
					<DateRangePicker
						{...props}
						...
						<div className="DateRangePicker__picker DateRangePicker__picker--horizontal DateRangePicker--with-caret">
							<div className="DayPicker DayPicker--horizontal">
								<div className={styles.rangePickerIn}>
									{ranges.map((range) => (
										<RangeButton
											key={this.rangeId(range)}
											id={this.rangeId(range)}
											activeRange={stateValue}
											range={range.range}
											clickFunction={this.onPresetRangeSet}
										>
											{range.label}
										</RangeButton>
									))}
									<RangeButton
										id="custom_range"
										params="startDate"
										clickFunction={this.onCalendarFocusChange}
									>
										{customRangeLabel}
									</RangeButton>
								</div>
